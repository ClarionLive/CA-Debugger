...
function Wait-Paused([int]$t) { $sw=[Diagnostics.Stopwatch]::StartNew(); while ($sw.Elapsed.TotalSeconds -lt $t) { while ($script:cursor -lt $sink.Count) { $l=$sink[$script:cursor]; $script:cursor++; Note $l; if ($l.Length -gt 600) { Write-Host ($l.Substring(0,600)+'...[trunc]') } else { Write-Host $l }; if ($l -match '"event":"paused"') { if ($l -match '"ebp":"(0x[0-9A-F]+)"') { $script:ebp=$matches[1] }; if ($l -match '"va":"(0x[0-9A-F]+)"') { $script:va=$matches[1] }; return $true }; if ($l -match '"event":"exited"') { return $false } }; if ($proc.HasExited) { return $false }; if ($MenuItem -and -not $script:poked) { $cp = Get-Process ([IO.Path]::GetFileNameWithoutExtension($Target)) -ErrorAction SilentlyContinue | Select-Object -First 1; if ($cp) { $r=[MenuPoke]::Poke($cp.Id,$MenuItem); if ($r) { $script:poked=$true; Write-Host "## $r" } } }; Start-Sleep -Milliseconds 100 }; return $false }
if ($PrePauseSec -gt 0) { $sw0=[Diagnostics.Stopwatch]::StartNew(); while ($sw0.Elapsed.TotalSeconds -lt $PrePauseSec) { if ($MenuItem -and -not $script:poked) { $cp = Get-Process ([IO.Path]::GetFileNameWithoutExtension($Target)) -ErrorAction SilentlyContinue | Select-Object -First 1; if ($cp) { $r=[MenuPoke]::Poke($cp.Id,$MenuItem); if ($r) { $script:poked=$true; Write-Host "## $r" } } }; Start-Sleep -Milliseconds 200 }; Drain; Write-Host ">>> pause"; $proc.StandardInput.WriteLine("pause") }
